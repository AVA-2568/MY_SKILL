...
2. caveman 介入：
   - 只把 thinking-first 的\"理解 + 最小干预\"两层产物**压成 3-5 短要点**
...
### pre-think 压缩白名单（关键边界）

pre-think **只压缩 thinking-first 的"理解 + 最小干预"两层产物**：
- 理解层：任务目标、衡量标准、信息缺口 → 压成要点
- 最小干预层：是否"顺手加了未要求内容" → 压成要点

**以下两层不进入压缩，原样透传给 Domain skill（与 thinking-first 纪律 2/3 不冲突）：**
- **源锚定层**：每个事实性断言的溯源（已读文件 / 工具结果 / 官方文档）必须保留
- **不确定性层**：`推测` / `未知` 标签必须保留

原因：源与不确定性是安全/正确性红线，压缩会丢失关键信息。caveman 砍的是"为完整而写的废话"，不是溯源与不确定声明。
